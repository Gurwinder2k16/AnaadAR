...
public class NewBehaviourScript_Heritage_Walk : MonoBehaviour{

		int score=0;
		int max_score=100;
	    public Text loc;
		static NewBehaviourScript_Heritage_Walk mins;
	    Stack<string> stk=new Stack<string>();

	public GameObject check;
	string checked_level=null;
	string target;
	    public static NewBehaviourScript_Heritage_Walk instance 
	    {
		    get 
		    {
			if (mins == null) 
			{
				mins = new NewBehaviourScript_Heritage_Walk ();
            }
			return mins;
			}
		}
	...
	void sub_process(int score,string target)
	{
		if (!stk.Contains (target)) {
			stk.Push (target);
			set_score (get_score () + 100);
			Debug.Log (target + " " + 100);
			if (done ()) {
